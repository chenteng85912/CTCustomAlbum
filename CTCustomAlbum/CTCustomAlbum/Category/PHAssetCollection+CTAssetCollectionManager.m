...
- (void)fetchCollectionInfoWithSize:(CGSize)size
    ...
    //开始截取最新一张照
    PHAsset *lastAsset = nil;
    if (CTPhotosConfiguration.showVideo) {
        lastAsset = assetResult.lastObject;
    }else {
        PHFetchResult *result = [self fetchCollectionAssets];
        for (NSInteger i = assetResult.count-1; i>=0; i--) {
    ...
    [[PHCachingImageManager defaultManager] requestImageForAsset:lastAsset targetSize:newSize contentMode:PHImageContentModeAspectFill options:nil resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
